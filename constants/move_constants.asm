...
OLD_MOVES EQU const_value + -1
	...
	const INK_SPRAY    ; 107 needs anim
	const MEGAPHONE    ; 108
	const BLINDSIDE    ; 109 needs anim
	const WILL_O_WISP  ; 10a
	const EVIL_EYE     ; 10b needs anim
	const UPPERCUT     ; 10c needs anim
	const TERRORIZE    ; 10d needs anim
	const LUNGE        ; 10e needs anim
	const DRAGON_CLAW  ; 10f needs anim
	const SHELL_BOMB   ; 110 needs anim
	const MAGNETISM    ; 111 needs anim
	const MAGNET_BOMB  ; 112
	const LAVA_BURST   ; 113
	const ECHO         ; 114
	const TAIL_SLAP    ; 115
	const SHADOW_PUNCH ; 116
...
const_value = -NUM_SPECIAL_ANIMS
	...
	const ANIM_CONFUSED          ; -17 (ffe9)
	const ANIM_SLP               ; -16 (ffea)
	const ANIM_BRN               ; -15 (ffeb)
	const ANIM_PSN               ; -14 (ffec)
	const ANIM_SAP               ; -13 (ffed)
	const ANIM_FRZ               ; -12 (ffee)
	const ANIM_PAR               ; -11 (ffef)
	const ANIM_IN_LOVE           ; -10 (fff0)
	const ANIM_IN_SANDSTORM      ;  -f (fff1)
	const ANIM_IN_NIGHTMARE      ;  -e (fff2)
	const ANIM_IN_WHIRLPOOL      ;  -d (fff3)
; battle anims
	const ANIM_MISS              ;  -c (fff4)
	const ANIM_ENEMY_DAMAGE      ;  -b (fff5)
	const ANIM_ENEMY_STAT_DOWN   ;  -a (fff6)
	const ANIM_PLAYER_STAT_DOWN  ;  -9 (fff7)
	const ANIM_PLAYER_DAMAGE     ;  -8 (fff8)
	const ANIM_WOBBLE            ;  -7 (fff9)
	const ANIM_SHAKE             ;  -6 (fffa)
	const ANIM_HIT_CONFUSION     ;  -5 (fffb)
	const ANIM_IN_RAIN           ;  -4 (fffc)
